...
const OCR_QUEUE_FILE_ICON_SVG =
  '<svg class="ocr-queue-file-icon" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>';
const OCR_QUEUE_ARCHIVE_ICON_SVG =
  '<svg class="ocr-queue-file-icon" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="3" y="4" width="18" height="4"/><path d="M5 8h14v12H5z"/><path d="M10 12h4"/></svg>';
const OCR_QUEUE_DOWNLOAD_ICON_SVG =
  '<svg class="ocr-queue-action-icon" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 3v12"/><path d="m7 10 5 5 5-5"/><path d="M5 21h14"/></svg>';
...
function allDoneWithZip() {
  return ocrQueue.length > 0 && ocrQueue.every((item) => item.status === "done" && Boolean(item.zipId));
}
...
function updateOcrOverlayForSelection() {
  const item = getSelectedOcrItem();
  const shouldShow = Boolean(ocrInFlight && item && (item.status === "pending" || item.status === "processing"));
